...
y3 = [zeros(2*tN,1);y1(1:end-2*tN,1)];
y4 = [zeros(3*tN,1);y1(1:end-3*tN,1)];
yis = [];
for n = 1:length(y1)
    yis(n,1) = fcnZVDD(y1(n),y2(n),y3(n),y4(n));
end
...
function y = fcnZVDD(u1,u2,u3,u4)
    b = 0.1; 
